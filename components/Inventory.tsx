...
import { Package, Plus, Search, Filter, MoreVertical, Edit, Trash2, Minus, X, AlertTriangle, ChevronDown, ArrowUpDown, Tag, Layers, ArrowRight } from 'lucide-react';
...
const Inventory: React.FC = () => {
  ...
  const [newProduct, setNewProduct] = useState<Partial<Product>>({
    name: '', barcode: '', costPrice: 0, salePrice: 0, stock: 0, minStock: 5, category: 'Geral'
  });
  ...
  const handleSaveProduct = async () => {
    ...
    await loadProducts();
    setShowModal(false);
    setNewProduct({ name: '', barcode: '', costPrice: 0, salePrice: 0, stock: 0, minStock: 5, category: 'Geral' });
    setEditingId(null);
    ...
    <div className="flex flex-col gap-8 h-[calc(100vh-8rem)] max-w-[1700px] mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex-1 min-h-0 flex flex-col gap-6">
        ...
        <div className="glass-card rounded-[2.5rem] p-6 border-white/20 flex flex-col md:flex-row gap-4 items-center shadow-2xl shadow-slate-200/20">
          ...
          <div className="flex items-center gap-3">
             <div className="p-4 bg-indigo-600/10 text-indigo-600 rounded-2xl flex items-center gap-3">
                <Package size={18} />
                <span className="text-[10px] font-black uppercase tracking-widest">{filteredProducts.length} Itens filtrados</span>
             </div>
          </div>
        </div>
                    ...
                    <tr key={p.id} className="group hover:bg-white/40 dark:hover:bg-slate-800/40 transition-all duration-300">
                      <td className="px-10 py-6">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 bg-slate-50 dark:bg-slate-950 rounded-2xl flex items-center justify-center text-slate-300 group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-sm">
                            <Package size={20} />
                          </div>
                          <div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[3rem] shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-white/10">
            ...
            <div className="p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <button
                        ...
                        className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-950 border-2 border-transparent hover:border-indigo-200 rounded-xl text-left flex justify-between items-center font-bold text-slate-800 dark:text-white shadow-inner transition-all"
                      >
                        <span className="truncate">{newProduct.category || 'Selecionar...'}</span>
                        <ChevronDown size={20} className={`text-slate-300 transition-transform ${isCatDropdownOpen ? 'rotate-180' : ''}`} />
                      </button>

                      {isCatDropdownOpen && (
                        <div className="absolute top-[calc(100%+8px)] left-0 w-full glass rounded-3xl border border-white/20 shadow-2xl z-[120] p-4 space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
                          <div className="relative">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Preço de Custo (R$)</label>
                    <input 
                      type="number" 
                      className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-950 border-2 border-transparent focus:border-indigo-600 rounded-xl outline-none font-black text-slate-800 dark:text-white shadow-inner"
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 text-indigo-500">Preço de Venda (R$)</label>
                    <input 
                      type="number" 
                      className="w-full px-4 py-3 bg-indigo-50/30 dark:bg-slate-950 border-2 border-transparent focus:border-indigo-600 rounded-xl outline-none font-black text-xl text-indigo-600 shadow-inner"
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-rose-500 uppercase tracking-widest ml-1">Aviso Estoque Mínimo</label>
                    <input 
                      type="number" 
                      className="w-full px-4 py-3 bg-rose-50/30 dark:bg-slate-950 border-2 border-transparent focus:border-rose-500 rounded-xl outline-none font-black text-rose-600 shadow-inner"
