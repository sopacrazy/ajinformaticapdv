...
import { Truck, Plus, Search, Edit, Trash2, X, Phone, MapPin, Mail, FileText, ArrowRight } from 'lucide-react';
...
const Suppliers: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      await api.saveSupplier(formData);
    }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-slate-800 dark:text-white tracking-tighter uppercase mb-1">
            Gestão de <span className="text-gradient">Fornecedores</span>
          </h1>
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-[0.3em]">
            Lista de distribuidores e parceiros
          </p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md z-10">
              <tr className="border-b border-indigo-50/5">
                <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Fornecedor</th>
                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">CNPJ</th>
            ...
            <tbody className="divide-y divide-indigo-50/5">
              {filteredSuppliers.map((supplier) => (
                <tr key={supplier.id} className="group hover:bg-white/40 dark:hover:bg-slate-800/40 transition-all duration-300">
                  <td className="px-10 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        <span className="font-black text-slate-800 dark:text-white text-base tracking-tight">{supplier.name}</span>
                        <div className="flex items-center gap-1.5 mt-0.5">
                          <MapPin size={10} className="text-slate-400" />
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest truncate max-w-[200px]">{supplier.address || 'Sem endereço'}</span>
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    {supplier.cnpj ? (
                      <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-800/50 rounded-xl w-fit">
                        ...
                        <span className="text-[10px] font-black text-slate-600 dark:text-slate-300 uppercase tracking-widest">{supplier.cnpj}</span>
                      </div>
                    ) : '—'}
                  </td>
                  <td className="px-8 py-6">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[3rem] shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-white/10">
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2 space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Endereço Fiscal / Sede</label>
                  <input 
                    type="text" 
                    className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-950 border-2 border-transparent focus:border-indigo-600 rounded-xl outline-none font-bold text-slate-800 dark:text-white shadow-inner"
              ...
              <div className="pt-6 border-t border-indigo-50/10 flex justify-end gap-3">
                ...
                >
                  Salvar Fornecedor <ArrowRight size={16} />
                </button>
              </div>
            </form>
          </div>
